...
_**集合可以用二进制表示， 二进制从低到高第 i 位为 1 表示 i 在集合中，为 0 表示 i 不在集合中。**_

- 例如集合 {0,2,3} 可以用二进制数 1101(2)表示；反过来，二进制数 1101(2)就对应着集合 {0,2,3}。

_**正式地说，包含非负整数的集合 S 可以用如下方式「压缩」成一个数字：** f(S) = Σ_{i∈S} 2^i_
...
特别地，**只包含最小元素的子集**，即二进制最低 1 及其后面的 0, 即lowbit.
可以用 s & -s 算出。举例说明：
...
![二进制表示](Assets/daily_010_bit_004.png)
