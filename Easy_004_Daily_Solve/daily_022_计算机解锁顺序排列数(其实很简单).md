--- Create Time: 2025-12-10 10:00:00
--- @Author: xcyy
...
- 可以使用编号为 j 的计算机的密码解锁编号为 i 的计算机，其中 j 是任何小于 i 的整数，且满足 complexity[j] < complexity[i]（即
  j < i 并且 complexity[j] < complexity[i]）。
...
class Solution:
    def countPermutations(self, complexity: List[int]) -> int:
        ...
        # j 可以解锁 其右侧 密码复杂度比自身大 的机器

        n = len( complexity )

        for i in range( 1, n ):
            # 若complexity[ i ] <= complexity[ 0 ]: 无法破解
            if complexity[ i ] <= complexity[ 0 ]: return 0

        # 0号可以破解所有密码,方法数为1~n-1号位的排列数
        MOD = 1_000_000_007
